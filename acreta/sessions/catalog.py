"""Session catalog + durable queue for Acreta 004 core runtime."""
...
from acreta.adapters import registry as adapter_registry
from acreta.config.logging import logger
from acreta.config.settings import get_config, reload_config
...
if __name__ == "__main__":
    previous_data_dir = os.getenv("ACRETA_DATA_DIR")
    try:
        with tempfile.TemporaryDirectory(prefix="acreta-catalog-selftest-") as tmp:
            os.environ["ACRETA_DATA_DIR"] = tmp
            reload_config()
            init_sessions_db()
            run_id = "selftest-run"
        ...
        if previous_data_dir is None:
            os.environ.pop("ACRETA_DATA_DIR", None)
        else:
            os.environ["ACRETA_DATA_DIR"] = previous_data_dir
        reload_config()
