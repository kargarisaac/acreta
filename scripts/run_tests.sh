...
Environment overrides are also supported (e.g. ACRETA_LLM_PROVIDER).
USAGE
}
...
print_section "Acreta test runner"
...
export ACRETA_LLM_PROVIDER="$LLM_PROVIDER"
export ACRETA_LLM_MODEL="$LLM_MODEL"
if [[ -n "$LLM_BASE_URL" ]]; then
  export ACRETA_LLM_BASE_URL="$LLM_BASE_URL"
fi
export ACRETA_LLM_FALLBACK_PROVIDER="$LLM_FALLBACK_PROVIDER"
export ACRETA_LLM_FALLBACK_MODEL="$LLM_FALLBACK_MODEL"
if [[ -n "$LLM_FALLBACK_BASE_URL" ]]; then
  export ACRETA_LLM_FALLBACK_BASE_URL="$LLM_FALLBACK_BASE_URL"
fi

export ACRETA_PROVIDER="$AGENT_PROVIDER"
export ACRETA_MODEL="$AGENT_MODEL"
export ACRETA_AGENT_FALLBACK_PROVIDER="$AGENT_FALLBACK_PROVIDER"
if [[ -n "$AGENT_FALLBACK_MODEL" ]]; then
  export ACRETA_AGENT_FALLBACK_MODEL="$AGENT_FALLBACK_MODEL"
fi

export ACRETA_EMBEDDINGS_PROVIDER="$EMBEDDINGS_PROVIDER"
export ACRETA_EMBEDDING_MODEL="$EMBEDDINGS_MODEL"
...
run_integration() {
  ...
  export ACRETA_INTEGRATION=1
  export ACRETA_LLM_INTEGRATION=1
  export ACRETA_EMBEDDINGS_INTEGRATION=1
...
run_quality() {
  ...
  "$PYTHON_BIN" -m compileall -q acreta
