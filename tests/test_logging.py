...
from acreta.config import logging as logging_mod
...
def test_loguru_messages_do_not_use_percent_style_placeholders() -> None:
    source_root = Path(__file__).resolve().parents[1] / "acreta"
    pattern = re.compile(
    ...
    assert not violations, "Found percent-style placeholders in loguru logger calls:\n" + "\n".join(violations)


def test_compact_source_returns_repo_relative_path() -> None:
    path = str(Path(__file__).resolve())
    assert logging_mod._compact_source(path) == "tests/test_logging.py"


def test_compact_source_handles_site_packages_path() -> None:
    source = "/opt/homebrew/lib/python3.11/site-packages/claude_agent_sdk/_bundled/claude"
    assert logging_mod._compact_source(source) == "claude_agent_sdk/_bundled/claude"


def test_log_filter_suppresses_claude_sdk_spam_by_default(monkeypatch) -> None:
    monkeypatch.delenv("ACRETA_LOG_CLAUDE_SDK", raising=False)
...
def test_log_filter_can_enable_claude_sdk_spam(monkeypatch) -> None:
    monkeypatch.setenv("ACRETA_LOG_CLAUDE_SDK", "1")
    record = {"message": "Using bundled Claude Code CLI: /tmp/claude"}
