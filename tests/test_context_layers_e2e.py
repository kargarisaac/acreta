...
class TestContextLayersE2E(unittest.TestCase):
    def test_chat_loads_context_docs_when_memory_is_thin(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("acreta.app.cli.get_config", return_value=config),
                patch("acreta.app.cli.search_memory", return_value=[]),
                patch("acreta.app.cli.AcretaAgent", _FakeAgent),
            ):
